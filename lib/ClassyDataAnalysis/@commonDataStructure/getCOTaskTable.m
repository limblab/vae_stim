...
% check all databurst versions
dbVerCheck = NaN(numTrials,1);
for trial = 1:numTrials
    ...
    if ~isempty(idxDB)
        dbVerCheck(trial) = cds.databursts.db(idxDB,2);
    else
        warning('getCOTaskTable:badDataburstVersion',['This trial (' num2str(trial) ') has no databursts: ',num2str(cds.databursts.db(idxDB,2))]);
    end
end
if length(unique(dbVerCheck)) == 1 && unique(dbVerCheck) ~= 1
    % this means the DB version is wrong
...
for trial = 1:numTrials
    ...
    if cds.databursts.db(idxDB,2)==1
        % Occasionally there's a corrupt databurst with a bad version,
        % even though overall the version is correct. So skip that
        % trial
        ...
        warning('getCOTaskTable:badDataburstVersion',['This trial (' num2str(trial) ') has weird databurst of version: ',num2str(cds.databursts.db(idxDB,2))]);
    end
end
