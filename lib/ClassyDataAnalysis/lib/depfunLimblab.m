function [trace_list, builtins, matlab_classes, prob_files, prob_symbols,  ...
                eval_strings, called_from, opaque_classes] = depfunLimblab(varargin)
%DEPFUN  Locate dependent functions of program file.
%   
%   This is a renamed copy of the old matlab function depfun, which was
%   removed in release 2015b. depfun executes MUCH faster than the current
%   version of matlab.codetools.requiredFilesAndProducts. A few references
%   in warning message strings have been changed, and depreciation warnings
%   deleted, but this function is otherwise identical to the 2015a
%   implementation
%
%    DEPFUN will be removed in a future release. 
...
%--------------------------------------------------------------------------
    ...
    %warning(message('MATLAB:DEPFUN:DeprecatedAPI'));
        ...
        while i <= nin
            if isa(varargin{i},'char')
                switch varargin{i}
                  ...
                  case '-nographics'
                    warning(message('DEPFUN_LIMBLAB:Deprecated', 'nographics'));
                  case '-alwayslog'
                    warning(message('DEPFUN_LIMBLAB:Deprecated', 'alwayslog'));
                  ...
                  case '-print'
                    if i < nin
                        ...
                        error(message('DEPFUN_LIMBLAB:NoPrintFile'));
                    end
                  case '-notrace'
                      warning(message('DEPFUN_LIMBLAB:DeprecatedNotrace'));
                  case '-expand'
                    expand = true;
                  case '-calltree'
                    calltree = true;
            ...
            elseif isa(varargin{i},'cell')
                ...
                error(message('DEPFUN_LIMBLAB:WrongClass', i));
            end
            i = i + 1;
        end

        if narg_files == 0
            error(message('DEPFUN_LIMBLAB:NoTraceArguments'));
        else
    ...
    function analyze_trace_all
        ...
        for i=1:newdepfun_nlhs
            switch i
                ...
                otherwise
                    error(message('DEPFUN_LIMBLAB:InternalError'));
            end
        end
    end
    %---------------------------------------------------------------------------
...
%---------------------------------------------------------------------------
    function output_report
        ...
        if fid < 0
          error(message('DEPFUN_LIMBLAB:CannotOpenFileForWrite', report_file));
        end
        ...
        if toponly
          toponly_msg = getString(message('DEPFUN_LIMBLAB:FormatTopOnly'));
        else
          toponly_msg = '';
        end 
        ...
        for i=1:noutput
          switch i
            case 1                % trace list
              ...
              %====================== Files not on MATLABPATH: 
              format = getString(message('DEPFUN_LIMBLAB:ReportFilesNotOnMATLABPATH'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(off_path)
              ...
              %====================== Handle Graphics factory callback names:
              if ~isempty(tmp_files)
                  format = getString(message('DEPFUN_LIMBLAB:ReportHandleGraphicsFactoryCallbackNames'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
            ...
            case 2                % builtin list
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportBuiltinList')) ;
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(builtins)
            ...
            case 3                % MATLAB classes
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportMATLABClasses'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
            ...
            case 4                % problem list
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportProblemList'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(prob_files)
            ...
            case 5                % problem symbols: NOT IMPLEMENTED
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportProblemSymbolsNOTIMPLEMENTED'));
              fprintf(fid,format);
              %----------------------------------------------------------
            case 6                % eval strings:
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportEvalStrings'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
            ...
            case 7                % called from / call list
              if ~calltree
                  %----------------------------------------------------------
                  format = getString(message('DEPFUN_LIMBLAB:ReportCalledFromListbyTraceList'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
                  ...
                  %----------------------------------------------------------
                  format = getString(message('DEPFUN_LIMBLAB:ReportCallListbyTraceList'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
            ...
            case 8                % opaque classes
              %----------------------------------------------------------
              format = getString(message('DEPFUN_LIMBLAB:ReportOpaqueClasses'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
        ...
        for i=1:noutput
          switch i
            case 1
              ...
              data{n  ,2} = [];
              if toponly
                data{n+1,2} = getString(message('DEPFUN_LIMBLAB:FormatTopOnly'));
              else
                data{n+1,2} = '';
              end 
              data{n+2,2} = [];
    ...
    function [cbnames,cbstrings] = analyze_fig_file(figfile,k)
        ...
        fn = fieldnames(data);
        if length(fn) > 1
            warning(message('DEPFUN_LIMBLAB:BadHGStructure', figfile));
            return
        end
        hg_struct = data.(fn{1});
        ...
        if k==1
            if ~quiet
                format = getString(message('DEPFUN_LIMBLAB:ReportGeneratingHandleGraphicsCallbackNamesToAnalyzefigFil'));
                fprintf(format);
            end
            hg_cbnames = create_hg_cbnames;
            if ~quiet
                format = getString(message('DEPFUN_LIMBLAB:ReportDone'));
                fprintf(format);
            end
        end
    ...
    for j=1:nfield
        ...
        if any(strcmp(hg_cbnames,lstructnames{j}))
            if ischar(fi)
                ...
                cbstrings{end+1} = fi;
            elseif ~isempty(fi)
                warning(message('DEPFUN_LIMBLAB:NotAStringForCallback', strtrim( lstructnames{ j } )));
            end
        elseif isstruct(fi)
...
function [trace_file,off] = next_arg_file(arg_file)
    ...
    if ~isempty(fullname)
        if isdir(fullname)
            error(message('MATLAB:DEPFUN:FileIsADirectory', arg_file));
        end
        trace_file = arg_file;
        return
    end
    ...
    if ~isempty(f) && isempty(e)
        ...
        if ~isempty(fullname)
            ...
            if isempty(e)
                if isdir(fullname)
                    error(message('DEPFUN_LIMBLAB:FileIsADirectory', arg_file));
                end
                trace_file = [arg_file '.'];
                return
            end
    ...
    error(message('DEPFUN_LIMBLAB:FileDoesNotExist', arg_file));
end
...
function fn = stdpath(file,root)
    ...
    if ndirs < 0
        error(message('DEPFUN_LIMBLAB:PathBeyondRoot', file));
    end
end
